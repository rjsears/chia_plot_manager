...
def space_report() -> str:
    ...
    table.add_row(f'Total Number of Plots on [green]{chianas.hostname}[/]', f'[yellow]{get_all_available_system_space("used")[1]}[/]')
    ...
    table.add_row(f'Maximum # of plots when full', f'[yellow]{get_all_available_system_space("total")[1]}[/]')
    table.add_row(f'Plots completed in the last 24 Hours', f'[yellow]{plots_last_day}[/]')
    table.add_row(f'Average Plots per Hour', f'[yellow]{round(chianas.current_total_plots_daily / 24, 1)}[/]')
    table.add_row(f'Average Plotting Speed Last 24 Hours (TiB/Day)', f'[yellow]{round((chianas.current_total_plots_daily * int(plot_size_g) / 1000), 2)}[/]')
    if replace_plots:
        table.add_row(f'Days to fill/replace all current drives/plots', f'[yellow]{days_to_fill}[/]')
    else:
        table.add_row(f'Days to fill all current drives', f'[yellow]{days_to_fill}[/]')
    table.add_row(f'Current Plot Storage Drive', f'[yellow]{current_plot_drive}[/]')
    table.add_row(f'Temperature of Current Plot Drive', f'[yellow]{current_plot_drive_temp}°C[/]')
    table.add_row(f'Latest Smart Drive Assessment of Current Plot Drive', f'[yellow]{current_plot_drive_smart_assesment}[/]')
