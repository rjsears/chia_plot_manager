...
"""
Part of Harvester Health Check
"""
...
import sys
import os
sys.path.append('/home/chia/harvester_health')
import yaml
...
def setup_logging(default_path='/home/chia/harvester_health/logging.yaml', default_level=logging.CRITICAL, env_key='LOG_CFG'):
    """Module to configure program-wide logging. Designed for yaml configuration files."""
    log_level = read_logging_config('harvester_health_config', 'system_logging', 'log_level')
    log = logging.getLogger(__name__)
    level = logging._checkLevel(log_level)
    log.setLevel(level)
    system_logging = read_logging_config('harvester_health_config', 'system_logging', 'logging')
    if system_logging:
        path = default_path
